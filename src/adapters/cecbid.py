...
LIST_URLS = [
    ...
    "https://www.cecbid.org.cn/html/1/2/list1.htm",
    "https://www.cecbid.org.cn/html/1/3/list1.htm",
]

NOTICE_KEYWORDS = ("招标", "采购", "中标", "公告", "成交", "竞争性", "磋商", "询价")
SKIP_TITLES = ("首页", "联系我们", "登录", "注册", "English", "返回", "更多", "查看更多")


class CecbidAdapter(BaseAdapter):
    """中国招标投标网 — 首页及资讯列表。"""
    ...
    async def _parse_link(self, link, page, list_url: str, seen: set[str]) -> Optional[BidNotice]:
        ...
        parent = await link.evaluate_handle("el => el.closest('li, tr, div, p')")
        publish_date = None
        if parent:
            try:
    ...
    def _is_notice_link(self, href: str, title: str) -> bool:
        ...
        if host and "cecbid.org.cn" not in host:
            return False
        if not any(k in title for k in NOTICE_KEYWORDS):
            return False
        if ".htm" in href or "/html/" in href or "/info/" in href:
            return True
        return len(title) >= 12

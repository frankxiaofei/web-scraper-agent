"""中国通用招标网 (china-tender.com.cn) 适配器。"""
...
LIST_URLS = [
    "https://www.china-tender.com.cn/",
    "https://www.china-tender.com.cn/cms/channel/1xwgg/index.htm",
    "https://www.china-tender.com.cn/cms/channel/2ywgg/index.htm",
]

NOTICE_KEYWORDS = ("招标", "采购", "中标", "公告", "成交", "竞争性", "磋商", "预审", "结果")
SKIP_TITLES = ("首页", "联系我们", "登录", "注册", "快速导航", "友情链接", "查看更多")


class ChinaTenderAdapter(BaseAdapter):
    """中国通用招标网 — 招标采购公告列表。"""
    ...
    async def _parse_link(self, link, page, list_url: str, seen: set[str]) -> Optional[BidNotice]:
        ...
        publish_date = self._extract_date(title)
        parent = await link.evaluate_handle("el => el.closest('li, tr, div')")
        if parent and not publish_date:
            try:
    ...
    def _is_notice_link(self, href: str, title: str) -> bool:
        ...
        if host and "china-tender.com.cn" not in host:
            return False
        if not any(k in title for k in NOTICE_KEYWORDS):
            return False
        if "/cms/" in href or ".htm" in href or ".html" in href:
            return True
        return len(title) >= 15

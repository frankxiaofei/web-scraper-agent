...
LIST_URLS = [
    "http://www.bidchance.com/",
    "https://www.bidchance.com/",
    "http://www.bidchance.com/zbgg/",
    "https://www.bidchance.com/zbgg/",
]

NOTICE_KEYWORDS = ("招标", "采购", "中标", "公告", "成交", "竞争性", "磋商", "询价", "预告")
SKIP_TITLES = ("首页", "联系我们", "登录", "注册", "English", "返回", "更多", "会员")


class BidchanceAdapter(BaseAdapter):
    """中国招标网 — 招标公告列表。"""
    ...
    async def _scrape_list_page(self, list_url: str, max_items: int) -> list[BidNotice]:
        ...
        async with self.browser.new_page() as page:
            try:
                response = await page.goto(list_url, wait_until="domcontentloaded", timeout=45000)
            except Exception as e:
                logger.debug("列表页加载失败 %s: %s", list_url, e)
                return []
    ...
    def _is_notice_link(self, href: str, title: str) -> bool:
        ...
        if host and "bidchance.com" not in host:
            return False
        if not any(k in title for k in NOTICE_KEYWORDS):
            return False
        if ".htm" in href or ".html" in href or "/zbgg/" in href or "/info/" in href:
            return True
        return len(title) >= 12
